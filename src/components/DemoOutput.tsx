...
const TimelineItem = ({ item, isLast, index }: { item: typeof sotorasibTimeline[0]; isLast: boolean; index: number }) => (
  ...
  >
    <div className="flex flex-col items-center">
      ...
      />
      {!isLast && <div className="w-px flex-1 mt-1" style={{ background: "hsl(0 0% 22%)" }} />}
    </div>
    <div className="pb-5">
        ...
        {item.badge && (
          <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
            item.badge === "Warning"
              ...
              : ""
          }`} style={item.badge !== "Warning" && item.badge !== "Potential" ? { background: "hsl(0 0% 100% / 0.12)", color: "hsl(0 0% 90%)" } : {}}>
            {item.badge}
          </span>
        )}
      </div>
      <p className="text-xs mt-0.5" style={{ color: "hsl(0 0% 45%)" }}>{item.desc}</p>
    </div>
  </motion.div>
);
...
const borderColor = "hsl(0 0% 15%)";
const textPrimary = "hsl(0 0% 92%)";
const textSecondary = "hsl(0 0% 45%)";
const accentColor = "hsl(0 0% 100%)";
const cardBg = "hsl(0 0% 6%)";
const hoverBg = "hsl(0 0% 10%)";
...
const DemoOutput = ({ darkMode }: { darkMode?: boolean }) => {
  ...
  return (
    <div ref={containerRef} className="rounded-lg" style={{ background: darkMode ? "hsl(0 0% 8%)" : undefined }}>
      {/* Header tags */}
      <motion.div
      ...
      >
        <span
          ...
          style={{ background: "hsl(0 0% 100% / 0.08)", color: "hsl(0 0% 65%)" }}
        >
      ...
      <div className="px-6 pb-4 grid grid-cols-2 lg:grid-cols-4 gap-3">
        {stats.map((s, i) => (
          <motion.div
            key={i}
            className="rounded-lg p-4 text-center cursor-default"
          ...
          >
            ...
            {s.sub && <p className="text-[10px] mt-0.5" style={{ color: "hsl(0 0% 35%)" }}>{s.sub}</p>}
          </motion.div>
        ))}
      </div>
        ...
        <div className="flex gap-6" style={{ borderBottom: `1px solid ${borderColor}` }}>
          {(["pathways", "regulatory"] as const).map((tab) => (
            <button
              ...
              className="text-sm pb-2.5 border-b-2 transition-all duration-300"
              style={{
        ...
        {activeTab === "pathways" && (
          ...
          >
            {compounds.map((compound, ci) => (
              <motion.div
                key={ci}
                className="rounded-lg p-5 cursor-pointer transition-colors duration-300"
                style={{
                  border: `1px solid ${expandedCard === ci ? "hsl(0 0% 25%)" : borderColor}`,
                  background: expandedCard === ci ? hoverBg : cardBg,
                  boxShadow: expandedCard === ci ? "0 0 30px -10px hsl(0 0% 100% / 0.06)" : "none",
                }}
            ...
            <div className="flex flex-wrap gap-2">
              {["AACT", "FDA Drugs@FDA", "FDA Designations", "SEC EDGAR", "AdCom Records"].map((src, i) => (
                <motion.span
                  ...
                  style={{ background: "hsl(0 0% 12%)", color: textSecondary }}
